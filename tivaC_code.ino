...
void SysTick_Handler(void) {
  sysTickCounter++;
  if (sysTickCounter >= 5) { // 5 seconds have passed
    if (fireDetected && !espTriggered) {
      ...
      for (int i = 0; i < 800000; i++) {}  // Short delay to ensure signal is recognized by ESP32
...
void setup() {
  ...
  NVIC_ST_RELOAD_R = 80000000 - 1; // Reload value for 1 second (80 MHz clock)
  NVIC_ST_CURRENT_R = 0; // Clear current value register
  NVIC_ST_CTRL_R = 7; // Enable SysTick with core clock and interrupts
}
...
void loop() {
  ...
  if (flameValue < flameThreshold) {
    if (!fireDetected) {
      // Fire just detected, start SysTick timer
      startSysTick = true;
      fireDetected = true;
    }
  } else {
    ...
    sysTickCounter = 0; // Reset SysTick counter
  ...
  for (int i = 0; i < 800000; i++) {} // Delay equivalent to delay(100)
}
